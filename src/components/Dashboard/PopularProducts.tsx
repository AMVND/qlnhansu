...
const popularProductsData: popularProductProps[] = [
    {
        id: '3432',
                    ...
                    popularProductsData.map((product)=>(
                        <Link to={`/product/${product.id}`} className='flex hover:no-underline'>
                            ...
                            <div className='ml-4 flex-1'>
                                ...
                                <span className={`text-sm font-medium ${product.product_stock === 0 ? 'text-orange-500': 'text-green-400'}`}>{product.product_stock === 0 ? "Out of stock" : product.product_stock + ' in stock'}</span>
                            </div>
